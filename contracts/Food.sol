...
contract Food {
    ...
    function getRestaurantOrders() public view returns(uint256[] memory){
    ...
    function getOrdererOrders() public view returns(uint256[] memory){
        uint256 totalMatch = 0;
        for(uint256 i=0; i< orders.length; i++){
            Order storage order = orders[i];
    ...
    function getOrder(uint256 orderId) public view returns(Order memory){
        Order storage order = orders[orderId];
        require(order.orderer == msg.sender || restaurants[order.restaurantId].owner == msg.sender);
